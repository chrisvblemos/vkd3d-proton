        ...
        D3D12_FEATURE feature, void *feature_data, UINT feature_data_size)
{
    ...
    switch (feature)
    {
        ...
        case D3D12_FEATURE_QUERY_META_COMMAND:
        {
            ...
            TRACE("input_data_size = %"PRIuPTR", input_data = %p, output_data_size = %"PRIuPTR", output_data = %p.\n",
            ...
            if (!memcmp(&data->CommandId, &IID_META_COMMAND_DSTORAGE, sizeof(data->CommandId)))
            {
                ...
                {
                    FIXME("Unexpected input/output sizes for DirectStorage meta command: %"PRIuPTR", %"PRIuPTR".\n",
        ...
        const D3D12_CONSTANT_BUFFER_VIEW_DESC *desc, D3D12_CPU_DESCRIPTOR_HANDLE descriptor)
{
    ...
    TRACE("iface %p, desc %p, descriptor %"PRIxPTR".\n", iface, desc, descriptor.ptr);
        ...
        D3D12_CPU_DESCRIPTOR_HANDLE descriptor)
{
    ...
    TRACE("iface %p, resource %p, desc %p, descriptor %"PRIxPTR".\n",
        ...
        const D3D12_UNORDERED_ACCESS_VIEW_DESC *desc, D3D12_CPU_DESCRIPTOR_HANDLE descriptor)
{
    ...
    struct d3d12_device *device = impl_from_ID3D12Device(iface);
    TRACE("iface %p, resource %p, counter_resource %p, desc %p, descriptor %"PRIxPTR".\n",
        ...
        D3D12_DESCRIPTOR_HEAP_TYPE descriptor_heap_type)
{
    ...
    TRACE("iface %p, descriptor_count %u, dst_descriptor_range_offset %"PRIxPTR", "
          "src_descriptor_range_offset %"PRIxPTR", descriptor_heap_type %#x.\n",
        ...
        const void *blob, SIZE_T blob_size, REFIID iid, void **lib)
{
    ...
    TRACE("iface %p, blob %p, blob_size %"PRIuPTR", iid %s, lib %p.\n",
        ...
        REFIID iid, void **meta_command)
{
    ...
    TRACE("iface %p, command_id %s, node_mask %#x, param_data %p, param_size %"PRIuPTR", iid %s, meta_command %p.\n",
        ...
        ID3D12Resource *target_resource, ID3D12Resource *feedback_resource, D3D12_CPU_DESCRIPTOR_HANDLE descriptor)
{
    ...
    TRACE("iface %p, target_resource %p, feedback_resource %p, descriptor %"PRIxPTR"\n",
...
{
    TRACE("iface %p, pBlob %p, Size %"PRIuPTR", iid %s, ppvDeserializer %p\n",
...
{
    TRACE("iface %p, pLibraryBlob %p, Size %"PRIuPTR", RootSignatureSubobjectName %s, iid %s, ppvDeserializer %p\n",
