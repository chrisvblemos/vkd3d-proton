...
static void STDMETHODCALLTYPE d3d12_bundle_ClearDepthStencilView(d3d12_command_list_iface *iface,
        ...
        UINT rect_count, const D3D12_RECT *rects)
{
    WARN("iface %p, dsv %"PRIuPTR", flags %#x, depth %.8e, stencil 0x%02x, rect_count %u, rects %p ignored!\n",
...
{
    WARN("iface %p, rtv %"PRIuPTR", color %p, rect_count %u, rects %p ignored!\n",
...
{
    WARN("iface %p, gpu_handle %"PRIx64", cpu_handle %"PRIxPTR", resource %p, values %p, rect_count %u, rects %p ignored!\n",
...
{
    WARN("iface %p, meta_command %p, parameter_data %p, parameter_size %"PRIuPTR" ignored!\n",
